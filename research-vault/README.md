...
`src/publish/check_claims.py` löst jeden Wikilink der Claim-Ebene gegen die Distillat-Ebene und gegen die Claim-Ebene selbst auf, mit Überschrift, wo der Link eine nennt. Wikilinks in Code-Auszeichnung, in Backticks oder in einem Code-Block, sind davon ausgenommen; sie nennen die Schreibweise und referenzieren nichts. Der `grounded`-Status dieser Ebene gilt nur, solange die Prüfung grün läuft.
