...
1. Open PRISM in read mode and click `Bearbeiten`. Only then enter a personal key of two to twelve letters, numbers, hyphens, or underscores. PRISM normalises the key to lowercase and uses `docs/data/screening/<key>.json` as the target. A normal reload returns to reading even with a saved profile; explicit trial, agent and verification URLs retain their working modes.
...
Schema 0.5 is the governed productive format. It adds embedded `provenance`, immutable `annotations`, deterministic `checks`, and `lifecycle` objects to every decision. Its status sequence is `identified → curated → agent-annotated → ai-agent-reviewed → verified → publication-approved`. The verification mode preserves the complete 0.5 envelope. An expert outcome can accept, correct and accept, request changes, or reject. Corrections append a version with `supersedes`, reason, actor, timestamp, and field-level differences. The event itself is written into the verifying expert's own file under `verification_schema` `femprompt-prisma-verification/0.1` and the array `verifications`, never into the agent file it judges; on load it is projected onto the agent track (ADR-040). The Literature Landscape follows `config/publication_policy.json`: the explicitly authorised preliminary release also admits source-bound AI-reviewed records with attributed review receipts, while preserving their actual lifecycle status. Without an explicit policy, the publisher requires `publication-approved`.
...
Earlier expert and model assessments come from `docs/data/research_vault_v2.json`. Human sessions show both as labelled references above the assessment; the LLM one can be taken over as category levels only (ADR-038). Agent runs render neither. Records below `tests/review-cases/` are acceptance fixtures and do not become research decisions through test execution.
