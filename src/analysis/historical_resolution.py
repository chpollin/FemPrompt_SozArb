...
def load_source_holds(repo: Path) -> dict[str, dict]:
    ...
    for work_id, entry in data.get("work_resolutions", {}).items():
        ...
        holds[work_id] = {
            "artifact": artifact,
            "sha256": artifact_hash(repo, artifact),
            "kind": "integrity_hold"
            if entry.get("integrity_hold")
            else "source_version_hold",
            **{
                key: entry[key]
                for key in ("reason", "agent_id", "model", "reviewed_at")
            },
        }
    return holds
...
def load_resolution(
    repo: Path, registry: dict, human: list[dict], inputs: set[str]
) -> dict:
    path = repo / RESOLUTION_PATH
    if not path.exists():
        return {}
    ...
    def row_matches(key, expected):
        ...
        if len(matches) != 1:
            raise ValueError(
                f"Historical resolution has a stale or ambiguous row: {key}"
            )
        return matches[0]
    ...
    def verify(entry):
        ...
        for source in evidence:
            ...
            if not source.get("locator") or (
                entry.get("outcome") == "resolved" and not source.get("source_url")
            ):
                raise ValueError(
                    "Historical resolution evidence lacks a source locator"
                )
            if source.get("quote"):
                text = safe_path(repo, reference.partition("#")[0]).read_text(
                    encoding="utf-8"
                )
                if " ".join(source["quote"].split()) not in " ".join(text.split()):
                    raise ValueError(
                        f"Historical resolution quote does not resolve: {reference}"
                    )
    ...
    for key, entry in data.get("identity_resolutions", {}).items():
        ...
        if entry.get("record_id") != key:
            raise ValueError(
                "Historical identity resolution key differs from its record"
            )
        row_matches(key, entry.get("historical_row_sha256"))
        if entry.get("outcome") != "resolved":
            continue
        expected = {
            "work_id": entry["target_work_id"],
            "version_id": entry["target_version_id"],
        }
        if key in index and index[key] != expected:
            raise ValueError(
                f"Historical record has a conflicting canonical binding: {key}"
            )
        if not entry.get("target_record_ids") or any(
            index.get(target) != expected for target in entry["target_record_ids"]
        ):
    ...
    for work_id, entry in data.get("work_resolutions", {}).items():
        ...
        if (
            ...
            or not entry.get("record_ids")
            or any(
                index.get(key, {}).get("work_id") != work_id
                for key in entry["record_ids"]
            )
        ):
            raise ValueError(
                f"Historical work resolution crosses identities: {work_id}"
            )
        ...
        for disposition in entry.get("administrative_record_dispositions", []):
            row = row_matches(
                disposition["record_id"], disposition["historical_row_sha256"]
            )
            if (
                disposition.get("kind") != "metadata_error"
                or row.get("Decision") != "Exclude"
                or index.get(disposition["record_id"])
                != disposition.get("target_registry_binding")
                or disposition.get("target_work_id") != work_id
            ):
