...
def _frontmatter(path: Path) -> tuple[dict, str]:
    ...
    parts = text.split("---", 2)
    return (
        (yaml.safe_load(parts[1]) or {}, parts[2])
...
def _historical_recovery_ris(issues: list[dict]) -> str:
    ...
    for issue in issues:
        ...
        if (
            issue.get("type") != "unmapped_human_record"
            or not resolution.get("external_identity_confirmed")
        ...
        lines.extend(
            [
                ...
                if metadata.get("type") == "proceedings-article"
                else "TY  - GEN",
                "TI  - " + titles[0],
            ]
        )
        for author in metadata.get("author", []):
            name = ", ".join(
                part for part in (author.get("family"), author.get("given")) if part
            )
            if name:
                lines.append("AU  - " + name)
        date = (
            metadata.get("published-online")
            or metadata.get("published-print")
        ...
        lines.extend(
            [
                "DO  - " + metadata["DOI"],
                "UR  - https://doi.org/" + metadata["DOI"],
                f"N1  - Recovered historical record {issue['record_id']}; metadata verified by {resolution['agent_id']}, {resolution['model']}, {resolution['reviewed_at']}. Import candidate only; no new screening or human verification. Historical decisions remain in assessment/human_assessment.csv. Evidence: {RESOLUTION_PATH}.",
                "ER  -",
                "",
            ]
        )
...
def _residual_progress(repo: Path, registry: dict, inputs: set[str]) -> dict[str, dict]:
    ...
    for key, entry in report["records"].items():
        if (
            entry.get("record_id") != key
            or registry["record_index"].get(key) != entry["canonical_binding_before"]
        ):
            raise ValueError(f"Stale residual identity binding: {key}")
        if not all(
            entry.get(field)
            for field in ("agent_id", "model", "reviewed_at", "finding", "next_step")
        ):
        ...
        for followup in entry.get("follow_up_reviews", []):
            if not all(
                followup.get(field)
                for field in ("agent_id", "model", "reviewed_at", "finding", "result")
            ):
                raise ValueError(f"Unattributed residual follow-up: {key}")
            if (
                datetime.fromisoformat(
                    followup["reviewed_at"].replace("Z", "+00:00")
                ).tzinfo
                is None
            ):
                raise ValueError(f"Residual follow-up needs a timezone: {key}")
            if (
                followup.get("canonical_binding_before")
                != entry["canonical_binding_before"]
                or artifact_hash(repo, followup["raw_metadata_reference"])
                != followup["raw_record_sha256"]
            ):
...
def _agent_records(
    ...
    for source in (
        reviews,
        getattr(reviews, "latest", {}),
        latest_reviews if latest_reviews is not None else {},
    ):
        for artifact, receipt in source.items():
            ...
            prior_at = (
                datetime.fromisoformat(prior["reviewed_at"].replace("Z", "+00:00"))
                if prior
                else None
            )
    ...
    for path in sorted((repo / "docs/data/screening").glob("*.json")):
        ...
        for record_id, record in sorted(data.get("decisions", {}).items()):
            ...
            projected, receipt, correction = reviewed_screening_projection(
                repo, artifact, record, outcomes
            )
            ...
            annotation = next(
                (
                    ...
                    if a.get("annotation_id") == record.get("active_annotation_id")
                ),
                None,
            )
            ...
            records[record_id].append(
                {
                    ...
                    "analysis": projected.get("analysis", {}).get("fields", {})
                    if correction
                    else body.get("analysis", {}).get("fields", {}),
                    "original_analysis": body.get("analysis", {}).get("fields", {}),
                    "current_ai_source_review": _review_summary(current_review),
                    "current_ai_review_result": current_review["result"]
                    if current_review
                    else "not_recorded",
                    "latest_original_ai_source_review": _review_summary(
                        outcomes.get(artifact)
                    ),
            ...
            if not annotation:
                issues.append(
                    {
                        "type": "missing_active_annotation",
                        "record_id": record_id,
                        "path": relative,
                    }
                )
...
def _assertions(
    repo: Path, inputs: set[str], reviews: dict | None = None
) -> list[dict]:
    reviews = reviews or {}
    result = []
            ...
            sources.append(
                {
                    ...
                    "anchor_present": bool(anchor and anchor in source_body),
        ...
        result.append(
            {
                ...
                "current_ai_source_review": _review_summary(
                    reviews.get(path.relative_to(repo).as_posix())
                ),
                "checked": {k: str(v) for k, v in metadata.get("checked", {}).items()},
                "sources": sources,
                "work_ids": sorted(
                    {source["work_id"] for source in sources if source["work_id"]}
                ),
                "structural_grounding_complete": bool(sources)
                and all(
                    s["exists"]
                    and s["anchor_present"]
                    and s["work_id"]
                    and s["version_id"]
                    for s in sources
                ),
...
def _work_rows(
    ...
    historical_bindings = {
        key: entry
        for key, entry in resolution.get("identity_resolutions", {}).items()
        if entry.get("outcome") == "resolved"
    }
    unmapped = [
        {
            "type": "unmapped_human_record",
            "record_id": key,
            "historical_rows": human_by_record[key],
            "identity_resolution": resolution.get("identity_resolutions", {}).get(key),
        }
        for key in sorted(human_by_record)
        if key not in index and key not in historical_bindings
    ]
    unmapped += [
        {"type": "unmapped_agent_record", "record_id": key}
    ...
    for work_id, record_ids in sorted(record_groups.items()):
        ...
        bound_legacy = {
            key: entry
            for key, entry in historical_bindings.items()
            if entry["target_work_id"] == work_id and key not in index
        }
        human_rows = [
            row
            for record_id in [*record_ids, *bound_legacy]
            for row in human_by_record[record_id]
        ]
        work_resolution = resolution.get("work_resolutions", {}).get(work_id, {})
        agent_rows = [
            row for record_id in record_ids for row in agents.get(record_id, [])
        ]
        human_decisions = sorted(
            {row.get("Decision", "") for row in human_rows if row.get("Decision")}
        )
        ...
        duplicates = [
            ...
            and row.get("Exclusion_Reason", "").strip().casefold() == "duplicate"
        ]
        metadata_ids = {
            item["record_id"]
            for item in work_resolution.get("administrative_record_dispositions", [])
        }
        substantive_human = [
            row
            for row in human_rows
            if row not in duplicates and row["Zotero_Key"] not in metadata_ids
        ]
        human_work_decisions = sorted(
            {
                row.get("Decision", "")
                for row in substantive_human
                if row.get("Decision")
            }
        )
        agent_decisions = sorted(
            {row["decision"] for row in agent_rows if row["decision"]}
        )
        open_reviews = sorted(
            {
                row["record_id"]
                for row in agent_rows
                if row.get("current_ai_review_result")
                in {"changes_requested", "unverifiable"}
            }
        )
        conflicts = []
        ...
        if (
            human_work_decisions
            and agent_decisions
            and human_work_decisions != agent_decisions
            and not (
                work_resolution.get("outcome") == "resolved"
                and work_resolution.get("resolution_kind", "").startswith(
                    "documented_round_specific"
                )
            )
        ):
        ...
        effective = (
            human_work_decisions[0]
            if len(human_work_decisions) == 1
            else agent_decisions[0]
            if not human_work_decisions and len(agent_decisions) == 1
            else None
        )
        if (
            work_resolution.get("outcome") == "resolved"
            and work_resolution.get("round1_effective_decision") != effective
        ):
            raise ValueError(
                f"Historical resolution differs from the retained human decision: {work_id}"
            )
        eligible_agent_rows = [
            row
            for row in agent_rows
            if row["decision"] == "Include" and row["state"] in STATES[3:]
        ]
        field_values, field_conflicts, field_undecidable = {}, {}, {}
        for name in sorted(
            {name for names in FIELDS.values() for name in names}
            | {"AN_Coding_Basis", "AN_Prompting_Role", "Studientyp"}
        ):
            ...
            undecidable = [
                row["undecidable"][name]
                for row in eligible_agent_rows
                if name in row["undecidable"]
            ]
            if undecidable:
        ...
        if conflicts:
            actions.append(
                "reconcile_recorded_divergences_without_overwriting_human_track"
            )
        if work_id in queue_by_work:
            actions.append(
                "execute_governed_source_screening"
                if queue_by_work[work_id].get("queue_status") == "ready"
                else "resolve_source_blockers_then_governed_screening"
            )
        if human_rows:
            actions.append(
                "retain_legacy_human_decision_and_document_historical_provenance_limits"
            )
        if agent_rows:
            actions.append(
                "retain_ai_review_tier_and_record_any_further_review_at_artifact_level"
            )
        if open_reviews:
            actions.append(
                "resolve_negative_source_review_before_using_affected_AI_coding"
            )
        ...
        result.append(
            {
                ...
                "human_duplicate_record_ids": sorted(
                    row["Zotero_Key"] for row in duplicates
                ),
                ...
                "human_record_dispositions": [
                    _human_disposition(row) for row in human_rows
                ],
                "effective_decision": effective,
                "decision_authority": "legacy_human"
                if human_work_decisions
                else "ai_review"
                if agent_decisions
                else "unassessed",
                "human_verification": "legacy_human_annotation_not_new_lifecycle_verification"
                if human_rows
                else "not_recorded",
                "agent_records": agent_rows,
                "legacy_human_provenance_note": "Consolidated historical expert CSV; no new named reviewer, review date, or lifecycle event inferred."
                if human_rows
                else None,
                "agent_lifecycle_states": sorted({row["state"] for row in agent_rows}),
                "current_ai_review_results": sorted(
                    {
                        row.get("current_ai_review_result", "not_recorded")
                        for row in agent_rows
                    }
                ),
                "open_ai_source_review_record_ids": open_reviews,
                "corrected_ai_record_ids": sorted(
                    {
                        row["record_id"]
                        for row in agent_rows
                        if row.get("applied_ai_correction")
                    }
                ),
                ...
                "analysis_eligible": effective == "Include"
                and bool(eligible_agent_rows)
                and not open_reviews
                and not source_hold
                and not any(
                    c.endswith("decisions") or c == "human_agent_decision_divergence"
                    for c in conflicts
                ),
                "analysis_fields": field_values,
                "analysis_conflicts": field_conflicts,
                "analysis_undecidable": field_undecidable,
                "conflicts": conflicts,
                "source_queue_status": queue_by_work.get(work_id, {}).get(
                    "queue_status"
                ),
...
def _attach_source_progress(work: dict, residual: dict, registry: dict) -> None:
    ...
    work["current_source_binding_record_ids"] = sorted(
        key for key in work["record_ids"] if key in bindings
    )
    work["newly_acquired_text_requires_binding"] = any(
        entry.get("original_text") and entry["record_id"] not in bindings
        for entry in progress
    )
    work["next_actions"].extend(
        entry["next_step"] for entry in progress if entry["record_id"] not in bindings
    )
    work["next_actions"].extend(
        review["recommended_disposition"]
        for entry in progress
        for review in entry.get("follow_up_reviews", [])
        if review.get("recommended_disposition")
    )
    ...
    for question, names in FIELDS.items():
        for name in names:
            ...
            available = [
                row
                for row in eligible
                if row["analysis_fields"].get(name)
                and name not in row["analysis_undecidable"]
                and all(value in allowed for value in row["analysis_fields"][name])
            ]
            for value in allowed:
                matching = [
                    row for row in available if value in row["analysis_fields"][name]
                ]
                output.append(
                    {
                        "question": question,
                        "field": name,
                        "value": value,
                        "work_count": len(matching),
                        "work_ids": [row["work_id"] for row in matching],
                        ...
                        "missing_or_unresolved_included_works": len(included)
                        - len(available),
...
def _candidate_rows(
    ...
    for candidate in sorted(
        package.get("records", []), key=lambda row: row["candidate_id"]
    ):
        ...
        alternative = (
            local_version.get("relation_to_preferred") == "alternative_recorded_version"
        )
        if alternative:
            conflicts.append("source_uses_alternative_publication_version")
        pending = candidate.get("source_enrichment", {}).get(
            "pending_zotero_corrections", []
        )
        if pending:
        ...
        result.append(
            {
                ...
                "selected_version_date": candidate.get("selected_version_date"),
...
def _followup_rows(followup: dict, registry: dict) -> list[dict]:
    ...
    if followup.get("schema") != "femprompt-targeted-followup/0.1" or not isinstance(
        followup.get("candidates"), list
    ):
        ...
        rows.append(
            {
                **candidate,
                ...
                "canonical_bindings": [dict(record_id=key, **binding)]
                if binding
                else [],
...
def build_package(repo: Path = REPO) -> dict:
    inputs = {
        ...
        "generated/agent-screening-queue.json",
        f"{SEARCH}/codex-websearch-2026-package.json",
        READY,
        FOLLOWUP,
        MANUSCRIPT,
        "knowledge/project.md",
        "src/analysis/build_completion_package.py",
        "src/file_hashing.py",
        "generated/conformance/conformance_map.yaml",
    }
    reviews = load_reviews(repo)
    ...
    schema = yaml.safe_load(
        (repo / "assessment/categories.yaml").read_text(encoding="utf-8")
    )
    agents, issues = _agent_records(
        repo, inputs, reviews, getattr(reviews, "latest", {})
    )
    ...
    source_records = [
        {"path": path, "sha256": _sha(repo / path)} for path in sorted(inputs)
    ]
    fingerprint = (
        "sha256:" + hashlib.sha256(_dump(source_records).encode("utf-8")).hexdigest()
    )
    registry_extra = [
        work
        for work in registry["works"]
        if work["work_id"] not in {row["work_id"] for row in works}
    ]
    return {
        ...
        "scope": {
            ...
            "already_identified_2026_candidates": True,
            "targeted_gap_fill_candidates": True,
            "cutoff_date_2026_package": package.get("cutoff_date"),
            "targeted_followup_search_date": followup.get("search_date"),
            "new_literature_search": "only_excellent_gap_fill_if_separately_justified",
            "target_membership_is_not_screening_inclusion": True,
        },
        ...
        "counts": {
            "canonical_records": len(registry["record_index"]),
            "canonical_record_bound_works": len(works),
            "registry_all_works": len(registry["works"]),
            "registry_works_without_canonical_record": len(registry_extra),
            "human_annotated_works": sum(
                bool(row["human_record_ids"]) for row in works
            ),
            "ai_review_recorded_works": sum(
                bool(row["agent_records"]) for row in works
            ),
            "works_with_conflicts": sum(bool(row["conflicts"]) for row in works),
            "works_with_open_ai_source_review": sum(
                bool(row["open_ai_source_review_record_ids"]) for row in works
            ),
            "records_with_accepted_ai_correction": sum(
                len(row["corrected_ai_record_ids"]) for row in works
            ),
            "unmapped_historical_human_records": sum(
                row["type"] == "unmapped_human_record" for row in issues
            ),
            "reconciled_historical_human_records": sum(
                len(row["historical_identity_bindings"]) for row in works
            ),
            "works_with_integrity_hold": sum(row["integrity_hold"] for row in works),
            "queued_source_works": len(queue.get("queue", [])),
            "source_queue_works_with_newly_acquired_unbound_text": sum(
                bool(work["source_queue_status"])
                and work["newly_acquired_text_requires_binding"]
                for work in works
            ),
            "canonical_decisions": dict(
                sorted(
                    Counter(
                        row["effective_decision"] or "unresolved" for row in works
                    ).items()
                )
            ),
            ...
            "candidates_2026_current_ready_source": sum(
                row["recorded_source_ready"] and row["source_hash_matches"]
                for row in candidates
            ),
            "targeted_followup_candidates": len(followup_candidates),
            "targeted_followup_with_canonical_binding": sum(
                bool(row["canonical_bindings"]) for row in followup_candidates
            ),
            "targeted_followup_recorded_fulltext_read": sum(
                row.get("source_access", {}).get("basis") == "fulltext_read"
                for row in followup_candidates
            ),
            "active_assertions": len(assertions),
            "active_assertions_with_current_ai_source_review": sum(
                bool(row["current_ai_source_review"]) for row in assertions
            ),
            "assertion_states": dict(
                sorted(Counter(row["state"] for row in assertions).items())
            ),
        },
        "works": works,
        ...
        "registry_candidates_without_canonical_record": [
            {
                "work_id": work["work_id"],
                "title": work["canonical_title"],
                "flags": work.get("flags", []),
                "review_status": work.get("review_status", {}),
                "legacy_work_ids": work.get("legacy_work_ids", []),
            }
            for work in registry_extra
        ],
        "analysis_tables": _analysis(works, schema),
        "analysis_field_definitions": schema["analysis_fields"],
...
"""
    ...
    for field in package["analysis_field_definitions"]:
        dictionary += (
            f"- `{field['name']}`: "
            + (", ".join(str(v) for v in field.get("values", [])) or "free text")
            + ("; multiple codes allowed" if field.get("multi") else "")
            + ".\n"
        )
    outline = (
        "# Provisional source-bound synthesis outline\n\n"
        + preamble
        + "\nThis is a writing and verification plan, not a finished synthesis. The canonical manuscript remains `"
        + MANUSCRIPT
        + "`. Tables count recorded coding; the statements below are existing assertion texts with their recorded authority, not new findings.\n\n"
    )
    topics = {
    ...
    for question, topic in topics.items():
        ...
        tables = [
            row for row in package["analysis_tables"] if row["question"] == question
        ]
    ...
    return {
        "completion-package.json": _dump(package),
        "README.md": readme,
        "data-dictionary.md": dictionary,
        "synthesis-outline.md": outline,
        "work-verification-queue.csv": _csv(
            package["works"],
            [
                "work_id",
                "title",
                ...
                "human_decisions",
                "human_substantive_decisions",
                "human_duplicate_record_ids",
                "human_metadata_error_record_ids",
                ...
                "current_ai_review_results",
                "corrected_ai_record_ids",
                "ai_correction_provenance",
                "open_ai_source_review_record_ids",
                "effective_decision",
                "analysis_eligible",
                "analysis_conflicts",
                "analysis_undecidable",
                "conflicts",
                "source_queue_status",
                "source_blockers",
                "source_acquisition_progress",
                "next_actions",
            ],
        ),
        "unmapped-historical-records.csv": _csv(
            [
                issue
                for issue in package["issues"]
                if issue["type"] == "unmapped_human_record"
            ],
            ["record_id", "type", "historical_rows"],
        ),
        "historical-recovery.ris": _historical_recovery_ris(package["issues"]),
        "candidate-2026-readiness.csv": _csv(
            package["candidates_2026"],
            [
                "candidate_id",
                "title",
                "doi",
                "selected_version_date",
                "source_lanes",
                "zotero_status",
                "canonical_bindings",
        ...
        "targeted-followup-queue.json": _dump(
            {
                "schema": "femprompt-targeted-followup-queue/0.1",
                "source_fingerprint": fingerprint,
                "scope": "separate_targeted_gap_fill_intake",
                "provenance": package["targeted_followup"],
                "candidates": package["targeted_followup_candidates"],
            }
        ),
        "targeted-followup-queue.csv": _csv(
            package["targeted_followup_candidates"],
            [
                "candidate_id",
                "title",
                "year",
        ...
        "registry-candidate-queue.csv": _csv(
            package["registry_candidates_without_canonical_record"],
            ["work_id", "title", "flags", "review_status", "legacy_work_ids"],
        ),
        "sq-analysis-tables.csv": _csv(
            package["analysis_tables"],
            [
                "question",
                "field",
                "value",
                "work_count",
                "work_ids",
                "denominator_field_coded_works",
                "denominator_ai_analysis_eligible_works",
                "denominator_recorded_include_works",
                "missing_or_unresolved_included_works",
                "authority",
                "counting_rule",
            ],
        ),
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        if stale:
            ...
            return 1
        print(
            f"Completion package {'current' if args.check else 'written'}: {len(outputs)} files"
        )
        return 0
