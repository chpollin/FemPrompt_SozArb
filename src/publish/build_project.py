...
OUTPUTS = (
    ...
    "docs/data/category_schema.json",
    "docs/data/analysis_fields.json",
    "docs/data/research_vault_v2.json",
    "docs/data/promptotyping_v2.json",
    "docs/data/fulltext_index.json",
    "docs/data/fulltext_manifest.json",
    "docs/data/literature_landscape.json",
    "docs/data/assertion_index.json",
    ...
    "generated/literature-readiness.json",
    "generated/literature-readiness.csv",
)
...
INPUT_GLOBS = (
    ...
    "generated/markdown_clean/*.md",
    "generated/distilled/**/*.json",
    "generated/distilled/**/*.md",
    "generated/source-acquisition/**/*.json",
    "generated/source-acquisition/**/*.md",
    "generated/source-acquisition/**/*.csv",
    "generated/source-acquisition/**/*.png",
    ...
    "generated/round2-intake.json",
    "generated/round2-agent-review/*.json",
    "research-vault/10_distillates/**/*.md",
    "research-vault/20_claims/**/*.md",
    "research-vault/20_distillates/**/*.md",
    "research-vault/30_assertions/**/*.md",
    "research-vault/40_output/**/*.md",
    "research-vault/references/*.json",
    "tests/review-cases/agent-runs/**/*.json",
    "docs/data/screening/*.json",
    "docs/data/knowledge_doc_bindings.json",
    "docs/data/*contract.json",
    "docs/data/concept_graph.json",
    "docs/vault/Papers/*.md",
    "docs/*.html",
    "docs/js/*.js",
    "docs/css/*.css",
    "docs/vendor/**/*",
...
def snapshot(repo: Path) -> dict[str, Any]:
    ...
    return {
        ...
        "inputs": {
            p.relative_to(repo).as_posix(): file_hash(p)
            for p in sorted(inputs, key=lambda item: item.relative_to(repo).as_posix())
        },
        "outputs": {
            p.relative_to(repo).as_posix(): file_hash(p)
            for p in sorted(outputs, key=lambda item: item.relative_to(repo).as_posix())
        },
...
def check_build(repo: Path = ROOT) -> None:
    ...
    if not path.is_file():
        raise ValueError(
            "No build manifest. Run python -m src.publish.build_project first."
        )
    ...
    load_reviews(repo)
    print(
        "OK: canonical inputs, generated data, review preparation and result site are current"
    )


def _run(module: str, *args: str) -> None:
    result = subprocess.run(
        [sys.executable, "-X", "utf8", "-m", module, *args],
        cwd=ROOT,
        capture_output=True,
        text=True,
        encoding="utf-8",
...
def build() -> None:
    ...
    for module in (
        "src.publish.build_category_schema",
        "src.publish.build_analysis_fields",
    ...
    for module in (
        "src.publish.generate_promptotyping_data_v2",
        "src.publish.build_screening_index",
        "src.acquire.build_codex_source_readiness",
        "src.analysis.build_agent_screening_queue",
        "src.publish.validate_research_vault",
        "src.publish.check_claims",
    ):
        _run(module)
    _run("src.publish.build_assertion_index")
    _run(
        "src.publish.generate_literature_landscape",
        "--policy",
        "config/publication_policy.json",
        "--verification-ledger",
        "generated/verification/ai-source-reviews.json",
    )
    _run("src.analysis.build_completion_package")
    _run("src.analysis.build_literature_readiness")
...
def main(argv=None) -> int:
    ...
    parser.add_argument(
        "--check",
        action="store_true",
        help="Read-only freshness and review-receipt check",
    )
