...
What can be discussed now is the methodological yield. The two-round design turns a common embarrassment, a review begun before reporting standards existed, into a usable contrast. The retrospective record shows precisely which trAIce and PRISMA items are recoverable from data and which cannot be recovered, and the prospective round shows the same items falling out of recorded data by construction. The boundary runs where decisions stop being data, where prose notes, unversioned prompts, and memory do not survive while per-decision fields do. Read as a design specification for epistemic infrastructure, understood as the chain of documented, re-executable transformations that must surround an opaque model step for its outputs to circulate as knowledge claims, the boundary yields five lessons, each with its concrete instance in the review's repository:

1. Pre-specification is unrecoverable. A protocol that was not written down before the run can only be declared absent afterwards; round one's protocol is exactly such a declared absence (trAIce M1), and round two's update protocol was therefore committed to the repository before the first search ran (`knowledge/update-protocol.md`).
2. Acquisition provenance must be captured at the moment of the run. Round one's PDF acquisition survives only in its outcomes, and post-hoc recovery corroborated a minority of records, which marks the ceiling of reconstruction; the round-two protocol therefore commits the raw per-lane search exports as part of the record at run time.
3. Every decision needs its actor and its evidence basis recorded alongside it. Round one's human decisions survive as one consolidated table without per-reviewer attribution, a named gap; in PRISM each decision is written to a per-reviewer decision file whose Git commit author is the reviewer, with the pinned text passage as its recorded basis (ADR-021, ADR-024).
4. Controlled vocabularies must be enforced at the input surface. Round one's exclusion-reason column carries empty and out-of-vocabulary values because the spreadsheet did not constrain them, visible in the generated flow model; the PRISM import bridge rejects such values with a validation report before they enter the record.
5. A reported figure must carry a re-executable derivation path from the raw data. A number that exists only as typed prose has no such path, drifts as the data changes, and cannot be audited; in this review every round-one figure is regenerated by the committed replay script, which self-tests against the canonical benchmark (`src/replay/`), and the running text points to the data files rather than restating values.
