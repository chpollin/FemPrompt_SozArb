...
What does the research literature offer practitioners and educators in social work who want to use generative AI in a gender- and bias-sensitive way, and how must prompt engineering be adapted for that field? We report a literature review conducted in two rounds over a corpus of 326 papers identified through four commercial deep research systems and manual search. Every paper was assessed in two parallel tracks: a consolidated human expert annotation in a spreadsheet workflow, and a batch LLM assessment, recorded separately and never merged. [PENDING: one to two sentences with the headline synthesis findings along SQ1 to SQ3, after analysis coding.] Beyond its findings, the review doubles as a methodological case: we attempted to render the completed first round as a PRISMA 2020 and PRISMA-trAIce record, and the attempt failed in instructive ways. A protocol cannot be pre-specified after the fact; search provenance could be corroborated for only 34 of 326 records; and an adversarial recomputation overturned our own documented interpretation of the near-chance agreement between the two tracks (Cohen's kappa 0.056), a correction possible only because exclusion reasons had been recorded per decision: 72 of the 108 papers the LLM included against a human exclusion were corpus-management exclusions invisible to a model that sees one paper at a time, and restricted to content judgments the include rates converge (68.3 against 67.3 percent). The second round therefore runs under a pre-committed protocol through an instrument that derives the trAIce flow artifact and the AI disclosure from recorded data. The methodological contribution is the documented boundary between what reconstruction recovers and what must be recorded at the moment of the run.
...
The paper makes two kinds of contribution. The first is substantive: the synthesized findings of the review along three sub-questions, the inventory of prompting techniques reaching this literature and the evidence behind them (SQ1), the mapping of bias axes to proposed and tested mitigations (SQ2), and the domain-specific constraints and gaps that general-purpose prompt engineering guidance does not cover (SQ3). [PENDING: one-sentence preview of the main findings, after coding.] The second is methodological: a transparent account of the two-round design, including the retrospective record of round one with every gap named, the decomposed analysis of human-LLM screening divergence, and the instrument whose data model derives the PRISMA-trAIce flow artifact and the AI disclosure section from recorded data, to our knowledge, as of June 2026, the first tool documented to do so. The methodological account is deliberately told as a sequence of decisions and corrections, including the one where our own infrastructure proved our documented interpretation wrong.
...
PRISM, the screening and documentation layer built on the review's repository, is a static, dependency-free web tool over the published research data. Capture stays in the experts' spreadsheet; an import bridge converts the spreadsheet export into per-reviewer decision files with a validation report (vocabulary violations, empty reasons, duplicates, unknown identifiers), and PRISM is the downstream layer for evidence (reading, searching, and pinning text passages as per-category evidence), reconciliation of divergent decisions, and record generation. Its data model keeps AI and human decisions as separate sibling records and derives the PRISMA-trAIce adapted flow diagram (trAIce item R1) and a consolidated AI disclosure section (trAIce M-items plus RAISE Table 1 elements) from the recorded data rather than from hand-filled templates. The retrospective record of round one, generated from the surviving data, names per item what a reader can and cannot verify: of PRISMA 2020's 27 items, 10 are reconstructable, 7 partially, 10 not; of the 17 trAIce items, 13 are reconstructable, 3 partially, and one, the pre-specified protocol, is missing by definition, because a pre-specification cannot be created after the fact.
...
## 5. Results: screening agreement and what the divergence is made of
...
## 6. Results: the literature on prompt engineering for social work, gender, and bias

[PENDING: this section is the substantive core of the results-led paper and follows from the analysis coding (3.5). Frame, per sub-question:]
