...
- Screening: the reviewing experts (R1, R2) screen in the established Excel workflow, this time with a reviewer column and enforced vocabulary; the result imports over the validation bridge; the offline LLM assessment runs with versioned prompt and recorded parameters as the sibling track; evidence grounding happens downstream in PRISM.
