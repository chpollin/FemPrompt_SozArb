...
from src.analysis.build_completion_package import _historical_recovery_ris, _work_rows
from src.analysis.historical_resolution import (
    RESOLUTION_PATH,
    load_resolution,
    load_source_holds,
)
...
def fixture(repo):
    registry = {
        "record_index": {
            key: {"work_id": "work:one", "version_id": "version:one"}
            for key in ["A", "B"]
        },
        "works": [
            {
                ...
                "preferred_version_id": "version:one",
            }
        ],
    }
    human = [
        {"Zotero_Key": "A", "Decision": "Exclude", "Notes": "Wrong author"},
        {"Zotero_Key": "B", "Decision": "Include"},
        {"Zotero_Key": "OLD", "Decision": "Include"},
    ]
    ...
    attribution = {
        ...
        "human_annotations_unchanged": True,
        "reason": "Publisher identifies this work.",
        "outcome": "resolved",
        "evidence": [
            {
                "source_path": "source.md",
                "sha256": artifact_hash(repo, "source.md"),
                "source_url": "https://example.org/study",
                "locator": "title",
                "quote": "A supported source identity.",
            }
        ],
    }
    identity = {
        **copy.deepcopy(attribution),
        "record_id": "OLD",
        "historical_row_sha256": record_hash(human[2]),
        "target_work_id": "work:one",
        "target_version_id": "version:one",
        "target_record_ids": ["A", "B"],
    }
    work = {
        **copy.deepcopy(attribution),
        "work_id": "work:one",
        "record_ids": ["A", "B"],
        "round1_effective_decision": "Include",
        "resolution_kind": "administrative_metadata_disposition",
        "administrative_record_dispositions": [
            {
                "record_id": "A",
                "historical_row_sha256": record_hash(human[0]),
                "kind": "metadata_error",
                "target_registry_binding": registry["record_index"]["A"],
                "target_work_id": "work:one",
            }
        ],
    }
    data = {
        "schema": "femprompt-historical-resolution/0.1",
        "identity_resolutions": {"OLD": identity},
        "work_resolutions": {"work:one": work},
    }
    save(repo, data)
...
def test_withdrawal_withholds_current_analysis_but_preserves_human_include(tmp_path):
    ...
    assert (
        rows[0]["effective_decision"] == "Include" and not rows[0]["analysis_eligible"]
    )
...
def test_reimported_historical_key_is_counted_once_and_must_match_identity(tmp_path):
    registry, human, _ = fixture(tmp_path)
    registry["record_index"]["OLD"] = dict(registry["record_index"]["B"])
    resolution = load_resolution(tmp_path, registry, human, set())
    rows, issues = _work_rows(registry, human, {}, {}, resolution)
    assert not issues
    assert rows[0]["human_record_ids"].count("OLD") == 1
    registry["record_index"]["OLD"]["version_id"] = "version:other"
    with pytest.raises(ValueError, match="conflicting canonical binding"):
        load_resolution(tmp_path, registry, human, set())
...
def test_recovery_ris_contains_confirmed_metadata_and_no_invented_identity():
    ...
    confirmed = {
        ...
        "record_id": "OLD",
        "identity_resolution": {
            "external_identity_confirmed": True,
            "external_identity": {
                "title": ["A recovered study"],
                "DOI": "10.1/test",
    ...
    assert (
        "EMPTY" not in ris and "no new screening" in ris and "AU  - Author, A." in ris
    )
...
def test_preview_serves_only_explicit_result_and_prism_roots(tmp_path):
    ...
    assert (
        resolve_target(tmp_path, "/results/index.html")
        == tmp_path / "build/site/index.html"
    )
    assert (
        resolve_target(tmp_path, "/prism/prisma.html") == tmp_path / "docs/prisma.html"
    )
    for url in (
        "/secret.txt",
        "/prism/../secret.txt",
        "/results/%2e%2e/%2e%2e/secret.txt",
        "/prism/C:/Windows/win.ini",
    ):
