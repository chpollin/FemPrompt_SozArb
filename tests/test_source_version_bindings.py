...
from src.analysis.work_versions import load_contract, source_binding_for_record, validate_registry
...
def test_manuscript_addition_preserves_record_and_preferred_bibliography(tmp_path):
...
def test_production_bindings_keep_vor_and_historical_source_holds():
    ...
    assert set(registry["source_index"]) == {"VSZM7CT6", "EXRF5629"}
    for key, binding in registry["source_index"].items():
