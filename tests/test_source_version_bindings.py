...
def source_fixture(tmp_path):
    ...
    vor = {
        "version_id": "version:vor",
        "title": "Example work",
        "version_type": "version_of_record",
        "version_date": "2022",
        "identifiers": {"doi": ["10.1/example"], "zotero_key": ["P1"]},
        "peer_review_status": "peer_reviewed",
        "peer_review_basis": "source_metadata",
        "integrity_status": "current",
        "relations": [],
    }
    registry = {
        "schema": "femprompt-work-version-registry/0.1",
        "works": [
            {
                "work_id": "work:one",
                "versions": [vor],
                "preferred_version_id": "version:vor",
                "latest_version_id": "version:vor",
            }
        ],
        "record_index": {"P1": {"work_id": "work:one", "version_id": "version:vor"}},
        "identifier_index": {
            "doi:10.1/example": {"work_id": "work:one", "version_id": "version:vor"}
        },
    ...
    binding = {
        "record_id": "P1",
        "work_id": "work:one",
        "bibliographic_version_id": "version:vor",
        "source_version_id": "version:am",
    ...
    am = {
        **deepcopy(vor),
        "version_id": "version:am",
        "version_type": "accepted_manuscript",
        "version_date": "2021",
        "identifiers": {"url": ["https://repository.test/manuscript"]},
        "relations": [{"type": "isVersionOf", "target_version_id": "version:vor"}],
    }
    entry = {
        "binding": binding,
        "version": am,
        "bibliographic_identity": {"title": "Example work", "doi": "10.1/example"},
        "agent_id": "agent",
        "model": "test",
        "reviewed_at": "2026-09-05T12:00:00Z",
        "reason": "The repository cover explicitly identifies this manuscript of this published work.",
        "evidence": [
            {
                "source_path": "source.md",
                "sha256": binding["source_sha256"],
                "locator": "cover",
                "quote": "Accepted Manuscript",
            }
        ],
    }
    manifest = {
        "schema": "femprompt-source-version-bindings/0.1",
        "records": {"P1": entry},
    }
    (tmp_path / "corpus/source_version_bindings.json").write_text(
...
def registered_source(tmp_path):
    registry, _manifest, binding = source_fixture(tmp_path)
    apply_source_bindings(tmp_path, registry, load_contract())
    (tmp_path / "corpus/work_version_registry.json").write_text(
        json.dumps(registry), encoding="utf-8"
    )
    return registry, binding


def test_screening_queue_accepts_bound_manuscript_and_rejects_wrong_served_version(
    tmp_path,
):
    from src.analysis.build_agent_screening_queue import build_queue

    _registry, binding = registered_source(tmp_path)
    ...
    paper = {
        ...
        "title": "Example work",
        "author_year": "Author 2022",
        "source_binding": binding,
    }
    (data / "research_vault_v2.json").write_text(
    ...
    assert "sha256:" + queue["sources"][
        "work_version_registry_sha256"
    ] == artifact_hash(tmp_path, "corpus/work_version_registry.json")
    source["P1"]["version_id"] = "version:vor"
    (data / "fulltext_manifest.json").write_text(json.dumps(source), encoding="utf-8")
    assert build_queue(tmp_path)["counts"]["ready_works"] == 0
    assert (
        "paper_source_version_mismatch" in build_queue(tmp_path)["queue"][0]["blockers"]
    )


@pytest.mark.parametrize("schema", ["0.1", "0.2"])
def test_manuscript_addition_preserves_record_and_preferred_bibliography(
    tmp_path, schema
):
    ...
    assert (
        registry["identifier_index"]["doi:10.1/example"]["version_id"] == "version:vor"
    )
    assert (
        registry["identifier_index"]["url:https://repository.test/manuscript"][
            "version_id"
        ]
        == "version:am"
    )
...
def existing_source_fixture(tmp_path, *, arxiv=True):
    ...
    registry["identifier_index"] = {
        "doi:" + doi: deepcopy(registry["record_index"]["P1"])
    }
    version_identity = {
        "title": "Example work",
        **({"arxiv": "2305.12345v2"} if arxiv else {"doi": doi}),
    }
    quote = (
        "Example work\narXiv:2305.12345v2 [cs.AI]"
        if arxiv
        else "Example work\nDOI: 10.1234/example"
    )
    (tmp_path / "source.md").write_text(
    ...
    binding.update(
        source_version_id="version:vor",
        source_version_type=version["version_type"],
        is_preferred_version=True,
        source_sha256=artifact_hash(tmp_path, "source.md"),
    )
    entry = {
        ...
        "agent_id": "source-qc-agent",
        "model": "test",
        "reviewed_at": "2026-09-05T12:00:00Z",
        "reason": "Title and exact version checked in the primary source.",
        "evidence": [
            {
                "source_path": "source.md",
                "sha256": binding["source_sha256"],
                "locator": "title and identifier on first page",
                "quote": quote,
            }
        ],
    }
    manifest = {
        "schema": "femprompt-source-version-bindings/0.2",
        "records": {"P1": entry},
    }
    write_source_manifest(tmp_path, manifest)
    return registry, manifest, binding
...
def write_source_manifest(repo, manifest):
    (repo / "corpus/source_version_bindings.json").write_text(
        json.dumps(manifest), encoding="utf-8"
    )
...
def test_existing_source_binding_changes_only_source_index(tmp_path, arxiv):
    ...
    validate_registry(registry, load_contract())
    assert {
        key: value for key, value in registry.items() if key != "source_index"
    } == before
    expected = {
        **binding,
        "binding_mode": "existing_version",
    ...
    assert set(inputs) == {
        tmp_path / "corpus/source_version_bindings.json",
        tmp_path / "source.md",
    }
...
def test_existing_nber_working_paper_binding_uses_registered_report_number(tmp_path):
    registry, manifest, _binding = existing_source_fixture(tmp_path, arxiv=False)
    version = registry["works"][0]["versions"][0]
    version.update(
        version_type="working_paper",
        identifiers={"url": ["https://www.nber.org/papers/w34255"]},
        landing_url="https://www.nber.org/papers/w34255",
    )
    identity = {
        "title": "Example work",
        "report_number": "34255",
    ...
    entry = manifest["records"]["P1"]
    entry["bibliographic_identity"] = identity
    entry["version_identity"] = deepcopy(identity)
    entry["binding"]["source_version_type"] = "working_paper"
    quote = "Example work\nNBER Working Paper 34255"
    (tmp_path / "source.md").write_text(quote, encoding="utf-8")
    source_hash = artifact_hash(tmp_path, "source.md")
    entry["binding"]["source_sha256"] = source_hash
    entry["evidence"][0].update(sha256=source_hash, quote=quote)
    write_source_manifest(tmp_path, manifest)

    apply_source_bindings(tmp_path, registry, load_contract())

    assert (
        source_binding_for_record(registry, "P1", tmp_path)["version_identity"]
        == identity
    )


@pytest.mark.parametrize("drift", ["url", "number", "type"])
def test_existing_nber_working_paper_binding_rejects_identity_drift(tmp_path, drift):
    registry, manifest, _ = existing_source_fixture(tmp_path, arxiv=False)
    version = registry["works"][0]["versions"][0]
    version.update(
        version_type="working_paper",
        identifiers={"url": ["https://www.nber.org/papers/w34255"]},
    ...
    entry = manifest["records"]["P1"]
    entry["bibliographic_identity"] = deepcopy(identity)
    entry["version_identity"] = deepcopy(identity)
    entry["binding"]["source_version_type"] = "working_paper"
    quote = "Example work\nNBER Working Paper 34255"
    (tmp_path / "source.md").write_text(quote, encoding="utf-8")
    ...
    entry["evidence"][0].update(sha256=source_hash, quote=quote)
    if drift == "url":
        entry["version_identity"]["url"] = "https://www.nber.org/papers/w99999"
    elif drift == "number":
        entry["version_identity"]["report_number"] = "99999"
    else:
        version["version_type"] = "unknown"
    write_source_manifest(tmp_path, manifest)
...
def test_existing_arxiv_binding_accepts_exact_identifier_without_doi(
    tmp_path, registered_location
):
    registry, manifest, _ = existing_source_fixture(tmp_path)
    version = registry["works"][0]["versions"][0]
    version["identifiers"] = (
        {"arxiv": ["2305.12345v2"]}
        if registered_location == "arxiv_id"
        else {"url": ["https://arxiv.org/pdf/2305.12345v2.pdf"]}
    )
    version.pop("landing_url")
    manifest["records"]["P1"]["bibliographic_identity"] = {
        "title": "Example work",
        "arxiv": "2305.12345v2",
    }
    write_source_manifest(tmp_path, manifest)
    apply_source_bindings(tmp_path, registry, load_contract())
    assert (
        source_binding_for_record(registry, "P1", tmp_path)["version_identity"]["arxiv"]
        == "2305.12345v2"
    )
...
@pytest.mark.parametrize(
    ...
    [
        ...
        "extra_entry_metadata",
        "extra_binding_metadata",
        "extra_identity_metadata",
        "foreign_work",
        "foreign_bibliography",
        "foreign_version",
        "same_work_other_version",
        "wrong_type",
        "wrong_preferred",
        "wrong_preferred_flag",
        "wrong_title",
        "wrong_doi",
        "no_identity",
        "wrong_revision",
        ...
        "arxiv_doi_only",
        "registry_unpinned",
        "registry_conflicting_revisions",
        "registry_conflicting_arxiv_id",
        "registry_malformed_arxiv",
        "registry_malformed_url",
        "missing_agent",
        "missing_model",
        "naive_timestamp",
        "bad_timestamp",
        ...
        "invented_quote",
        "missing_title_quote",
        "missing_revision_quote",
        "other_source_only",
        "unpinned_evidence_url",
        "wrong_evidence_revision",
        "malformed_evidence_url",
        "versionless_doi_url",
    ],
)
def test_existing_binding_rejects_identity_drift_and_unreviewed_sources(
    tmp_path, problem
):
    ...
    if problem == "old_manifest":
        ...
        other.update(
            ...
            version_date="2021",
            identifiers={"arxiv": ["2305.12345v1"]},
        )
...
def correction_fixture(tmp_path):
    ...
    original = {
        "work_id": "doi:10.1/example",
        "categories": {"AI_Literacies": 2, "Soziale_Arbeit": 2, "Fairness": 2},
        "decision": "Include",
        "analysis": {"fields": {"AN_Notes": "Original"}},
            ...
            key: [
                {
                    ...
                    "term": "This paper explores AI fairness and social work education.",
                    "snippet": "this paper explores AI fairness and social work education.",
                }
            ]
            for key in ("AI_Literacies", "Soziale_Arbeit", "Fairness")
        },
    }
    (tmp_path / "records.json").write_text(
        json.dumps({"decisions": {"P1": original}}), encoding="utf-8"
    )
    changes = [
        {
            "path": "/categories/Fairness",
            "before": 2,
            "after": 1,
            "reason": "Subordinate fairness reference.",
        }
    ]
    changes.extend(
        {
            "path": f"/evidence/{key}/0/term",
            "before": passage[0]["term"],
            "after": passage[0]["snippet"],
            "reason": "Exact manuscript capitalization.",
        }
        for key, passage in original["evidence"].items()
    )
    correction = {
        "paper_id": "P1",
        "base_artifact": "records.json#/decisions/P1",
    ...
    data = {
        "schema": "femprompt-screening-corrections/0.2",
        "corrections": {"P1": correction},
    }
    (tmp_path / "corrections.json").write_text(json.dumps(data), encoding="utf-8")
    receipt = {
        "artifact": "corrections.json#/corrections/P1",
        "sha256": record_hash(correction),
        "result": "accepted",
        "review_type": "ai-source-review",
        "agent_id": "reviewer",
        "model": "test",
        "reviewed_at": "2026-09-05T12:00:00Z",
        "findings": "Identity and all positive quotation evidence checked against the exact manuscript.",
        "canonical_binding": {"paper_id": "P1", **registry["record_index"]["P1"]},
        "source_binding": binding,
        "evidence": [
            {
                "source_path": "source.md",
                ...
                "version_id": "version:am",
                "locator": "body",
                "quote": "this paper explores AI fairness and social work education.",
            }
        ],
    }
...
def test_source_correction_keeps_history_and_binds_exact_version(tmp_path):
    ...
    projected, receipt, _ = reviewed_screening_projection(
        tmp_path,
        correction["base_artifact"],
        original,
...
def test_one_session_can_reject_original_and_accept_its_exact_correction(tmp_path):
    ...
    negative.update(
        artifact=correction["base_artifact"],
...
@pytest.mark.parametrize(
    ...
    [
        ...
        ("/evidence/Fairness/0/actor", "person"),
        ("/categories/Soziale_Arbeit", 1),
        ("/evidence/Fairness/0/snippet", "invented quote"),
    ],
)
def test_source_correction_rejects_unauthorized_paths_and_invented_quotes(
    tmp_path, path, after
):
    _, correction, data, ledger = correction_fixture(tmp_path)
    correction["changes"].append(
        {"path": path, "before": "unused", "after": after, "reason": "test"}
    )
    (tmp_path / "corrections.json").write_text(json.dumps(data), encoding="utf-8")
...
def test_source_receipt_cannot_relabel_another_version(tmp_path):
    ...
    ledger["reviews"][0]["source_binding"] = {
        **ledger["reviews"][0]["source_binding"],
        "source_version_id": "version:vor",
    }
...
def test_publisher_projects_manuscript_evidence_without_relabelling_bibliography(
    tmp_path,
):
    original, correction, _, ledger = correction_fixture(tmp_path)
    projected, receipt, _ = reviewed_screening_projection(
    ...
    registry = json.loads(
        (tmp_path / "corpus/work_version_registry.json").read_text(encoding="utf-8")
    )
    paper = {
        "id": "P1",
        "title": "Example work",
        "doi": "10.1/example",
        **generate_docs_data.work_version_projection("P1", registry),
    }
    schema = json.loads(
        (REPO / "docs/data/analysis_fields.json").read_text(encoding="utf-8")
    )
    result = generate_literature_landscape._validate_and_transform_record(
        "P1",
        projected,
        paper,
        schema,
        frozenset(projected["categories"]),
        receipt,
        repo=tmp_path,
    )
    ...
    assert all(
        passage["version_id"] == "version:am" and passage["source_path"] == "source.md"
        for category in result["categories"]
        for passage in category["evidence"]
    )
    ...
    with pytest.raises(ValueError, match="recorded version identity differs"):
        generate_literature_landscape._validate_and_transform_record(
            "P1",
            projected,
            paper,
            schema,
...
def test_fulltext_uses_bound_manuscript_with_separate_bibliography(
...
def test_metadata_identity_correction_suppresses_same_title_old_knowledge_doc(
    tmp_path, monkeypatch
):
    (tmp_path / "Example work.md").write_text(
        "Old synthetic summary of a different DOI", encoding="utf-8"
    )
    monkeypatch.setattr(generate_docs_data, "VAULT_PAPERS_DIR", tmp_path)
    meta = {
        "Title": "Example work",
        "DOI": "10.1/correct",
        "_metadata_correction": {
            "changes": [
                {"field": "DOI", "before": "10.1/wrong", "after": "10.1/correct"}
            ]
        },
    }
    paper = generate_docs_data.parse_llm_row(
        {"Zotero_Key": "P1"},
        {"P1": meta},
        {"P1": {"src": "raw", "source_file": "new-source.md"}},
    )
    assert paper["knowledge_doc"] is None
    assert paper["knowledge_coverage"] == "fulltext_ready"
    assert (
        paper["knowledge_doc_invalidation"]["reason"]
        == "bibliographic_identity_corrected"
    )
...
def test_production_bindings_keep_vor_and_historical_source_holds():
    ...
    for key in ("VSZM7CT6", "EXRF5629"):
        ...
        assert (
            registry["record_index"][key]["version_id"]
            == binding["bibliographic_version_id"]
        )
