...
Executed and green: `npm test` reports PASS 103/103 headless under jsdom (jsdom is a dev dependency, pinned in `package.json`). The browser leg (`run-tests.html`) runs the identical suite. The count dropped from 73 when ADR-017 removed the agreement section and rose with Sections K (analysis panel) and L (text source, load token, reconciliation).

## Relation to plan P1

Plan P1 names two test layers: the committed jsdom harness (node-based, dev dependencies allowed) and the browser runner. Both are now present and share one suite (`tests.js`): `run.mjs` plus `package.json` is the headless harness, `run-tests.html` the browser leg. Still open from P1: the additional acceptance checks remain a separate work item, namely export/import round-trip losslessness (FR-08), the seed reproducing the benchmark marginals from the real `research_vault_v2.json` rather than synthetic fixtures (FR-05), (the reviewer schema 0.1 to current migration is covered in Section H). The current suite covers the pure functions.
