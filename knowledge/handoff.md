...
Use the targeted identity/historical/source resolution reports and the regenerated completion queue. The mitigation-stage clarification and resolved C/D corrections are already documented; do not re-open them as blanket manual tasks. Preserve the Asseri withdrawal hold and the Latif source-version/human-decision conflict. Apply local metadata reconciliation to a future Zotero export deliberately, screen newly bound sources, and extend the active Assertion chain. AI corrections must retain original annotations and carry a newly reviewed sidecar artifact. Apply the explicit release policy without implying domain-expert verification. The Pages workflow is prepared but a local build does not deploy it or change the existing Pages configuration.

## Local-only source state

Third-party PDF binaries remain under the ignored local source directory. Their verified SHA-256 values are retained in the acquisition manifest. Versioned Markdown, repairs, visual assets, source audits, and readiness records provide the reproducible repository state. Temporary quality-control renderings remain ignored.

## Verification state

PRISM opens in read mode, including after reload with an existing reviewer profile. `Bearbeiten` enables reviewer and folder setup, annotations and saving; returning to reading preserves the unsaved draft without writing it. Explicit trial, agent and verification URLs retain their respective working modes; acceptance remains read-only. The native folder permission check and scholarly domain verification remain separate from automated browser checks.
...
The dated session close in `knowledge/journal.md` records the passing build, complete automated suites, browser pilot and independent repository-only Windows reproduction. Obsidian ACTIVE-WORK and the SocialAI knowledge documents point back to this repository for technical detail and keep the unfinished research work active. The session is secured on `codex/consistent-research-release`; this branch is not a deployment or a completed scholarly review.
