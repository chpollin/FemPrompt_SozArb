...
1. Evidence-grounded screening is the core. The screening view is built around the resolved reading text (since P2/ADR-024 the raw local full text under a connected clone, with the served knowledge document and the abstract as fallbacks, see [[data]]), an in-text and corpus-wide search, and pinning a search hit as a Beleg on a category. A category is not a bare checkbox, it carries the words that justify it, which is reproducible and matches the reviewing colleagues' method.
...
- Centre, reading column. Title and authors, a sticky in-text search bar (highlight all, step previous/next, hit counter, "Treffer anheften"), then the rendered document. The document is resolved through the single `fetchPaperText` seam (ADR-024), which takes the raw local full text from `generated/markdown_clean/` when a clone is connected, else the served `paper.knowledge_doc`, else the abstract; a source pill (Rohtext (lokal), Wissensdokument, nur Abstract) names what is being read, and the resolved source is recorded on the decision as `text_source`. The built-in minimal Markdown renderer (FR-11) splits the paper layer from the machine-extraction layer at the first `## Kernbefund` heading with a Volltext / KI-Extraktion toggle (ADR-016); the KI-Extraktion layer keeps coming from the knowledge document in raw mode.
...
Sync, opened on demand from the toolbar (ADR-020). The File System Access connect, reconnect, and reload; since ADR-024 the connect target is the repository root, from which the tool derives `docs/data/screening/` for the per-reviewer file (`<kuerzel>.json`, schema 0.2, diff-stable, sorted by paper id, ADR-021) and `generated/markdown_clean/` for the raw reading texts, with a hint when an old screening-only handle is picked. The panel also carries a commit-message generator that documents the session, export and import as the all-browser fallback, and the decision-log CSV. Who decided what is carried by the Git commit author; ADR-021 dropped the in-tool identity form, and ADR-014 removed the in-tool Git workflow (versioning stays in Git tooling outside the tool) and the reviewer reconciliation table.
...
PRISM is the binding screening surface for the project (ADR-019), and the reviewing colleagues screen in the tool. The literature review counts as complete only once its data has passed through PRISM under PRISMA 2020 and PRISMA-trAIce, so the most built-out surface, evidence-grounded in-tool screening, is the colleagues' working path. The Excel import bridge ([[specification]] P3 path, [[plan]]) survives as an entry and migration seam for a batch captured elsewhere, no longer as the canonical capture route. The pinning question is decided (2026-07-03): evidence pinning is technically optional per decision, expected where a category is contested and at reconciliation. The level of the synthesis surface (KI1, section 9) remains the open operator decision.

The evidence-basis item is resolved by P2 (ADR-024). The project's own model-by-input experiment had shown that knowledge documents amplify the inclusion tendency, so a Beleg pinned on a distillate inherits its framing; the tool now reads the raw local full text under a connected clone, records `text_source` on every decision, and labels the reading source visibly, which keeps the evidence basis auditable. In-text search on the corpus level still runs over the served index, and the per-source counts in the disclosure (trAIce M4) plus the human browser verification of the File System Access path remain open ([[plan]] P2 status).
