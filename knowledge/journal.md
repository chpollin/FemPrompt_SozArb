...
### 2026-07-18: Wissensbasis konsolidiert, achtzehn Dokumente auf zehn plus Anleitung

- **Ziel:** Die `knowledge/`-Dokumente auf eine kompakte, funktions- und lebenszyklusgeordnete Struktur zusammenführen, ohne Wissen zu verlieren, und die drei in der Sitzung entstandenen Arbeitsdateien (Ratifikations-Notiz, Replay-Design, Konformanzkarte) nicht als durables Wissen zementieren.
- **Verlauf:** Zuerst der Befund, dass die Wissensbasis mit phasengebundenen und Sitzungsartefakten wuchs, die je in ein Trägerdokument gehören. Sechs Zusammenführungen entlang gleicher Funktion und Lebensdauer, je ein Agent pro Zieldatei, damit keine zwei Agenten dieselbe Datei anfassen. `workflow.md` und die Prosa aus `r4-replay-design.md` nach `methods.md` (die Bau- und Laufdetails des Replays als `src/assess/README_replay.md` neben das Skript); `design.md` nach `specification.md`; `ratification-memo.md` in den Simulations-Ledger von `plan.md`; `analysis-fields-pilot.md` und `coding-concept.md` nach `update-protocol.md` (die E-Entscheide mit den F-Offenpunkten in einem Ledger); `research-vault-plan.md` und `distillate-check-plan.md` in das neue `research-vault.md`; `conformance-map.md` in das maschinenlesbare Artefakt `generated/conformance/conformance_map.yaml`, referenziert aus `standards.md`. Danach der repo-weite Link-Sweep, die verwaisten `[[Verweise]]` skriptgesteuert auf die neuen Heimatdokumente gemappt, die `related`-Frontmatter dedupliziert, `INDEX.md` von Hand auf die zehn Dokumente neu geschrieben, CLAUDE.md und die Restpfade in `data.md`, `prisma.js`, `evidence_audit.py` und den research-vault-READMEs nachgezogen.
- **Ergebnis:** `knowledge/` trägt zehn Dokumente plus die Anleitung. Kein verwaister Wikilink im Repo, das Konformanz-Artefakt ist valides YAML (PRISMA 2020, trAIce, RAISE), die Journal-Historie ihrer Dateiverschiebungen blieb als Prozessgedächtnis unangetastet. Working Tree, nichts committet.
- **Offen:** Die zwei phasengebundenen Dokumente `update-protocol.md` und `research-vault.md` lösen sich planmäßig weiter auf, ihr durabler Rest wandert nach `methods.md`, sobald Runde 2 startet beziehungsweise der Vault grounded ist. `workflow.md` und `methods.md` als Kandidatenpaar waren vor der Sitzung getrennt (Kette gegen Tiefe); die Zusammenführung fügt beide zu einem Dokument, dessen Kohärenz die nächste substanzielle Methodenarbeit prüft.
- **Dead Ends:** Die drei Arbeitsdateien als eigene Wissensdokumente zu behalten wäre der kürzere Weg gewesen, hätte aber Sitzungs- und Ausgabeartefakte mit durablem Wissen vermischt; sie gehören in ihr Trägerdokument beziehungsweise in die Daten. Die Journal-Prosa, die die alten Dateinamen als historische Verschiebung nennt, wurde bewusst nicht rückdatiert umgeschrieben, das würde den Record verfälschen.
...
- **Ziel:** Den Distillat-Bestand `generated/distilled/` auf die Fehlerklasse Modell-Paraphrase statt Zitat prüfen, deterministische Vorprüfung und adversariale Maschinenprüfung nach [[research-vault]], ohne ein Distillat zu verändern.
...
- **Ziel:** Den Top-Level-Ordner `research-vault/` nach [[research-vault]] anlegen und die vom Evidence-Audit als unauffällig ausgewiesenen Distillate in die Belegkette überführen, ohne die geschützten Ebenen anzufassen.
...
- **Ergebnis:** [[standards]] als R1-Deliverable, das Done-Kriterium erfüllt (jedes Item zeigt auf Daten oder eine benannte Lücke). Die zentrale retrospektiv unreparierbare Lücke, das fehlende vorab registrierte Runde-1-Protokoll (M1), ist benannt, nicht versteckt; die meta-analytischen PRISMA-Items (Risk of Bias, Effektmaße, Synthese, Certainty) sind als N/A für diesen Reviewtyp markiert. Branch committet und gepusht vor dem Rechnerwechsel; der Merge nach `main` bleibt die offene Menschenentscheidung (P0).
