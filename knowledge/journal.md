...
- **Konzeptrunde (2026-07-21, Frage-Antwort-Spiel mit dem Operator):** Doppelbeitrag mit Hierarchie, die methodische Integrationsfrage rahmt das Paper und SQ1 bis SQ3 sind der Ertragsnachweis des Workflows. Geltungsanspruch ist der dokumentierte Fall mit publizierten Artefakten, ohne Rahmenwerk-Anspruch; die Prinzipien in §7 sind Angebot an übernehmende Teams. Der Divergenzbefund dient als Motivation und als Demonstration dessen, was die aufgezeichnete Infrastruktur analysierbar macht; eine Warnungs-Lesart entfällt mangels Inter-Human-Baseline. Wirkabsicht ist die Übernahme des Workflows, dafür erhält die Discussion den Unterabschnitt "Integrating the workflow into a research process". Primäradressatin sind SW-Forschende, die ein LLM-gestütztes Review planen (JTHS-Fit). Draft v0.4 setzt das um; die fünf Lektionen sind auf drei Prinzipien verdichtet (Aufzeichnung im Moment des Laufs, Entscheidung als Datum mit Actor/Evidenz/Vokabular, re-exekutierbare Ableitungskette je publizierter Zahl).
...
- **Verlauf:** Ein Orchestrierungs-Workflow ließ drei Opus-Baustränge parallel arbeiten, danach je Strang eine adversarische Diff-Review plus ein Ausführungs-Check. (1) Das Replay (R2, TP3-Schritte 1 und 2) in `src/replay/replay_round1.py` paart die rohen Assessment-CSVs über Zotero_Key neu (der Merge-Bug vom März bleibt damit strukturell ausgeschlossen), baut das retrospektive FlowModel, trennt die Workflow-Ausschlüsse (Duplicate, No full text, Wrong publication type) für das Content-only-Subset, rechnet die Metriken je Track und Kategorie samt der 2x2-Sensitivität und reproduziert als Selbsttest die kanonische `agreement_metrics.json` exakt. Die V1-Konsequenzen (Content-only-Konvergenz als Base-Rate-Aussage, Kippen der besten Bedingung unter der Sensitivität) sind damit skriptgestützt, die README-Passage ist entsprechend korrigiert; Outputs `flow_model.json` und `agreement_replay.json` liegen unter `generated/benchmark-results/replay/`. (2) P2 (ADR-024) verlegt das Connect-Ziel auf die Repo-Wurzel; `fetchPaperText` bleibt die eine Naht und löst der Reihe nach Rohtext, Wissensdokument, Abstract auf; jede Entscheidung trägt `text_source` (additiv, Schema bleibt 0.2), die Leseansicht zeigt eine Quell-Pille, und die Paper-zu-Rohdatei-Zuordnung läuft titelpräfixbasiert im Browser. (3) `docs/onboarding.html` führt die beiden Reviewerinnen deutschsprachig Schritt für Schritt durch das Screening und ist aus `prisma.html` verlinkt. Die Review-Auflage des Onboarding-Strangs (transliterierte Umlaute in zitierten UI-Labels) wurde durch eine vollständige Umlaut-Korrektur der Seite geschlossen. Der V&V-Plan steht in plan.md, mit dem Replay als dritter Schicht der Testverantwortungsmatrix, einem Abschnitt zu den autonomen Prüfmaßnahmen (exit-code-gebundene Ausführungs-Checks, Label-Konsistenz per grep, Klicktests S1 bis S6 nach dem nächsten Deploy, Persona-Walkthrough des Onboardings) und den menschenpflichtigen Punkten (File System Access im echten Browser, Kolleginnen-Dry-Run, R5-Publikationsentscheidung).
- **Ergebnis:** Harness 83/83 grün (neue P2-Sektion), Replay-Selbsttest PASS, beide selbst ausgeführt statt nur aus Agentenberichten übernommen. Beide Code-Reviews ok mit Nits; drei von den Agenten aufgedeckte Punkte sind als Open items in plan.md erfasst (fehlender Reviewer-Umschalter im UI, ungetestetes Kollisionsverhalten des Rohdatei-Matchings, text_source-Race beim Erfassen während des Ladens). Strangweise committet auf main. Im Anschluss wurde auf Operatoranweisung der gesamte redigierte Markdown-Bestand gegen den gebauten Stand synchronisiert. In knowledge/ verlor die conformance-map den Draft-Caveat (die zählenden Items sind jetzt durch den committeten Replay behauptet), methods erhielt den Replay als kanonische Skriptreferenz samt Konditionen-Kontrast, design wurde auf ADR-024 und die entschiedene Pinning-Frage gezogen, INDEX erhielt den Replay-Glossareintrag. CLAUDE.md und das Root-README führen Replay und Onboarding; die Neben-READMEs wurden bereinigt (src/ neu als englischer Index, assessment/ samt human/ ohne Personennamen, volatile Zahlen und tote benchmark/-Pfade, docs/data/screening/ auf den Repo-Wurzel-Connect und das Schema mit text_source und override_reason, tests/ ohne den überholten P1-Absatz), und tests/manual-checklist.md wurde als das bislang fehlende P5-Deliverable angelegt.
- **Dead Ends:** Zwei Agenten-Rückgaben waren unbrauchbar (degeneriertes StructuredOutput, Retry-Cap), die Arbeit auf der Platte war jeweils vollständig und wurde durch eigene Ausführung und Review validiert; die Rückgabe eines Agenten ist kein Verifikationsersatz. Der P2-Plan sah ein Manifest aus `build_screening_index.py` vor, der Bau löst die Zuordnung stattdessen im Browser aus den gelisteten Dateinamen, wodurch kein zusätzliches Publikationsartefakt entsteht.
...
- **Ergebnis:** Alle 17 entscheidungsfreien Funde umgesetzt (plan, specification, design, standards, methods, update-protocol, conformance-map, journal-Reihenfolge 2026-03-24, tests/README), Frontmatter-updated gesetzt. Offen als Operatorentscheidung, in den Open items von [[plan]] geführt: O5 (eine geteilte Decisions-Datei oder eine pro Person; ADR-021-Default ist pro Person) und der Report-Verweis in conformance-map Item 1 (externes Forum-Paper versus paper/draft.md). Im selben Arbeitstag entstand vorab der aktualisierte Programm-Gesamtplan als Entwurfs-Atom im Vault ("2026-07-02 - Gesamtplan FemPrompt Aktualisierung (Entwurf)", Projects/SocialAI) samt Artifact-Übersicht; die Übernahme in den Gesamtplan-Master bleibt der Vault-Session vorbehalten, und die acht Ratifikationspunkte warten auf die Ergebnisse des Stakeholder-Treffens vom 01.07.
