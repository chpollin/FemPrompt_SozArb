...
The Evidence Companion now has an annotation-native Literature Landscape generated from the productive PRISM track. Its matrix and analysis profiles use only included records, preserve the ratified agent provenance, distinguish the reading basis, and drill down to the stored Paper evidence. The generator fails closed on invalid identities, category values, evidence, or Include analysis. The view regenerates as the productive track grows; final publication remains part of the complete-corpus Companion sync.

The static publication now consumes one generated category schema, uses pinned local runtime assets, and records Literature Landscape state in the URL. The corpus projection distinguishes records, stable works, document links, and distinct knowledge documents. Its source fingerprint replaces volatile generation metadata, and atomic publication preserves the previous artifact on failure. The stylesheet is separated into shared tokens, general Companion rules, Literature Landscape rules, and PRISM rules. Browser acceptance covers the public Companion independently of the PRISM pilot.

The canonical round-one replay is `src/replay/replay_round1.py`; the duplicate implementation under `src/assess/` has been retired. The ten-paper `ar2` result is stored as `ratified_agent_consensus`: two blind agent reviews, authentic PRISM trial exports, deterministic transcription and reconciliation checks, independent source adjudication, and explicit operator acceptance. The raw tracks remain unchanged and the productive records retain `actor: agent`. Milestone 1 is complete. Milestone 2 begins with duplicate/source-identity routing and the controlled expansion beyond these ten records. The historical phase sections below retain decision provenance; this operative state supersedes their older implementation-status sentences where they differ.
...
Status (2026-07-03, corrected 2026-08-22): steps 1 and 2 are built as the committed replay (`src/replay/replay_round1.py`). It re-pairs the raw CSVs by Zotero_Key, separates the workflow-criteria exclusions for the content-only subset, computes the metric set per track and per category with the 2x2 content-only sensitivity, requires the generated `Has_HA` flags to match the human CSV, and reproduces the canonical `generated/benchmark-results/agreement_metrics.json` as a self-test before writing `generated/benchmark-results/replay/`. The human-checked path is the reviewed script plus its self-test; an independent re-derivation by a second person remains open. Step 3, the analysis write-up as the licensed source for the paper, is [[analysis-divergence]] (2026-07-21), written from the replay outputs. The step 4 corrections are applied in `README.md`. The numbers themselves live in the data and the Evidence Companion, never in this plan.
...
Status (completed and ratified 2026-08-22): two governed blind reviews produced authentic PRISM exports for the same ten papers. A deterministic transcription verifier checks 20 records and 115 input evidence passages. Independent source adjudication produced a fixed consensus with seven Includes, one Unclear, two Excludes, 58 verified positive evidence passages, complete Include analyses, and no blocked source. Explicit operator acceptance promoted the consensus into `docs/data/screening/ar2.json` with `status: ratified_agent_consensus`; raw tracks, hashes, agent provenance, and the adjudication report remain versioned beside the run manifest.
...
- Rebuild the Companion data (research vault, categories, graph, divergences, annotation-native literature landscape) over the updated corpus; the human-AI comparison is updated as research material in the report layer and the Companion, never in the screening view.
- Regenerate `docs/data/literature_landscape.json` from each ratified productive PRISM track. Its matrix, analysis profiles, and evidence drill-down expose the source status and remain provisional only while the corresponding track lacks a binding acceptance status.
...
- Expand the ratified workflow beyond the first ten papers only after duplicate and source identities have been assigned deterministically; each batch must retain isolated tracks, Paper-level evidence, adjudication, and explicit productive integration provenance.
