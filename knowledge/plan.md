...
5. Done on 22 September 2026 (ADR-040): the verification path is reachable under the expert's own reviewer key, the event is written into the expert's file, the provenance renders readably and the agent record is headed as agent coding. The browser pilot carries the end-to-end check. Remaining is the field-level scope of item 1.
...
- Of the fifteen 2026 works without a usable full text, only the JSSWR paper on Chinese social work standards has an open-access author preprint, recorded in `generated/source-acquisition/codex-websearch-2026/reacquisition-2026-09-22.json` and converted locally without source QC or binding. The other fourteen are publisher-restricted and stay on the manual acquisition list `fehlende-volltexte-2026.xlsx`, which the partners received.
