...
The build is specified screen by screen in the Designsystem section below.
...
Interaktion. The daily editor loads the repository corpus, retains a browser recovery copy, and writes the selected reviewer file through one disk action. Historical conversion, reconciliation, disclosure, and exchange remain operator functions without editor-facing controls.

Grenzen. Collaboration is file-based through separate reviewer JSON files and GitHub Desktop. Concurrent tabs using the same reviewer key are outside the supported workflow.
...
Effekt. Implementiert (`src/publish/build_screening_index.py` build_machine_evidence, `docs/data/machine_evidence.json`; `docs/js/prisma.js` loadMachineEvidence, injectMachineEvidence, Ausschluss aus dem bindenden Count, kein Schreiben in die Reviewer-Datei). Die Provenienz-Kennzeichnung und die bindende Separierung sind test-abgedeckt (`tests/tests.js`). Schließt den proaktiven R2-Punkt (Maschinen-Evidenz vorladen), der nach ADR-016 offen blieb. Die Synthese-Ebene (KI1) bleibt davon unberührt und weiter offen. Superseded by ADR-022: the preload is removed, the model's reasoning was not paper evidence and fabricated a per-category provenance.
...
Effekt. Implementiert (`src/publish/build_fulltext.py`; `docs/js/prisma.js` fetchFullText/loadFulltextManifest/hasFullText, loadReadingInto combines the Volltext and LLM-Wissensdestillat layers, the availability pill; `.gitignore` fences the outputs). Current coverage and exclusions are derived from `docs/data/fulltext_manifest.json`. Corpus search stays on the committed distillation index for the same copyright reason.
...
Begründung. Kürzel und URL sind Laufidentität und gehören deshalb in die konkrete Zuweisung. Die Trennung von Persona und Laufparametern erlaubt denselben geprüften System-Prompt für mehrere unabhängige Instanzen. Getrennte Akteurs- und Quellenprovenienz beantwortet zwei verschiedene Fragen: wer hat codiert und auf welcher Textschicht beruht der Beleg. Eine explizite Zentralitätsentscheidung verhindert, dass eine Bedienhandlung die Include-Schwelle vorwegnimmt. Getrennte Trackpfade vermeiden konkurrierende Agentenschreibzugriffe.
...
Kontext. Der Abschlussaudit fand drei Datenrisiken. Test- und Produktionsmodus teilten denselben Browserzustand. Ein älterer Dateistand konnte nach einem fehlgeschlagenen Schreibvorgang neuere Browserdaten verdrängen. Bereits eingereihte Schreibvorgänge verwendeten den später veränderbaren Ordner-Handle. Außerdem schrieb der damalige Include-Ablauf zuerst den Screening-Record und ergänzte Analysefelder in weiteren Schreibvorgängen.

Wahl. Testmodus und Produktion erhalten getrennte `localStorage`-Namespaces. Beim Verbinden werden Datei- und Browserrecords anhand ihrer Record-Zeitstempel pro Paper zusammengeführt; neuere oder nur lokal vorhandene Records bleiben erhalten, während ein Konflikt ohne vergleichbare Zeitstempel sowie jede nicht lesbare Reviewer-Datei weitere Schreibvorgänge blockiert. Jeder eingereihte Schreibvorgang bindet den Handle, den Reviewer-Key und den serialisierten Stand zum Zeitpunkt der Einreihung. Ein Include zeigt die Analysefelder bereits im Arbeitsrecord und lässt die einzige Diskettenaktion erst zu, wenn Screening und Analyse gemeinsam vollständig sind. Das LLM-Wissensdestillat wird erst nach der eigenen gespeicherten Entscheidung verfügbar und der Lesemodus springt bei jedem Paperwechsel auf den Papertext zurück.

Begründung. Der Browsercache ist eine Wiederherstellungsschicht und darf einen fehlgeschlagenen physischen Schreibvorgang nicht in Datenverlust verwandeln. Eine unveränderliche Zielbindung verhindert, dass eine Warteschlange in einen später ausgewählten Ordner schreibt. Der vollständige Einzelschreibvorgang macht jeden gespeicherten Include-Record methodisch gültig und entspricht dem reduzierten Editorinnenfluss.

Effekt. Die Unit- und Browserprüfungen decken getrennte Testzustände, Merge-Regeln, fehlertolerante Schreibwarteschlangen, blockierende fehlerhafte Dateien, vollständige Includes vor dem Speichern und die geblendete Referenzschicht ab. Der native Berechtigungsdialog des Browsers bleibt der einzige manuelle Persistenztest.

### ADR-032 Full-text identity veto before publication (extends ADR-025/027)

Kontext. Der Abschlussaudit fand zwei falsche Volltextzuordnungen, die eine reine Titelähnlichkeit zugelassen hatte: einmal trug ein Text denselben Titel, aber abweichende DOI-, Autor- und Jahresangaben; einmal bestätigte nur der Dateiname die Korpusidentität, während der Dokumenttitel eine Pressemitteilung auswies. Ein falsch zugeordneter Volltext würde Screening und Belege auf ein fremdes Werk gründen.

Wahl. Der Publisher trennt Überschriften aus dem Dokument von Hinweisen im Dateinamen. Generische Überschriften werden verworfen. Ein Dateiname kann einen substanziellen Dokumenttitel mit mindestens zwei gemeinsamen Inhaltswörtern bestätigen, gilt allein jedoch nicht als Identitätsnachweis. Eine abweichende primäre DOI blockiert immer. Soweit der Volltext sie trägt, blockieren auch Konflikte bei Erstautor und Jahr eine Bestätigung über den Dateinamen. Eine ausdrücklich verworfene Datei darf nicht über den Autor-Jahr-Fallback erneut zugeordnet werden. Bei verbleibender Mehrdeutigkeit oder einem Titelkonflikt liefert PRISM den Metadaten-Abstract und weist keinen Volltext zu.

Begründung. Ein fehlender Volltext reduziert die Textbasis sichtbar; ein fremder Volltext erzeugt unsichtbar falsche Entscheidungen und Belege. Der Publisher entscheidet deshalb im Zweifel gegen die Zuordnung und hält die ungeklärten Fälle im Manifest prüfbar.

Effekt. Realfalltests blockieren die bekannten DOI-/Autor-/Jahr-Verwechslungen und die Pressemitteilung. Neun im technisch übernommenen Agententrack verwendete Volltexte bleiben byteidentisch. Der Build weist ungeklärte Fälle als `title_mismatch` oder `ambiguous` aus, ohne einen fremden Text auszuliefern.

### ADR-033 Canonical frontend schema and reproducible static publication

Kontext. Die zehn Kategorien waren parallel in Python und mehreren JavaScript-Dateien definiert. Die ausgelieferten Seiten bezogen Schriftarten und Laufzeitbibliotheken von externen Diensten. Der Companion setzte Record-Verknüpfungen mit Wissensdokumenten gleich. Der Datenpublisher schrieb einen Erzeugungszeitpunkt in die Ausgabe und verwendete einen festen temporären Dateinamen. Diese Kopplungen erschwerten eine reproduzierbare Prüfung und konnten auf GitHub Pages oder bei parallelen Builds zu abweichenden Zuständen führen.

Wahl. `assessment/categories.yaml` bleibt die kanonische Quelle. Ein deterministischer Publisher erzeugt `docs/data/category_schema.json`; PRISM, Companion und Literaturbild lesen dieselbe Projektion. D3 und Font Awesome werden mit Lizenzdateien lokal ausgeliefert. Systemschriften ersetzen Remote-Fonts. Der Knowledge Chat speichert den Schlüssel im Sitzungsspeicher des Tabs und weist auf die Übertragung von Frage und Forschungskontext hin. `research_vault_v2.json` verwendet einen Fingerabdruck der kanonischen Quellen, stabile Werkidentitäten und explizite Abdeckungszustände. Seine Veröffentlichung erfolgt über eine eindeutige temporäre Datei mit atomarem Austausch. Das Literaturbild schreibt Ansicht, Filter und Auswahl in die URL. Gemeinsame CSS-Tokens, allgemeine Oberfläche und Literaturbild-Stile liegen in getrennten Dateien.

Begründung. Eine generierte Kategorieprojektion verhindert inhaltliche Drift. Lokale Abhängigkeiten machen die statische Veröffentlichung unabhängig von Drittservern. Werkidentität und Abdeckungsstatus trennen Korpusstruktur von Dokumentbestand. Fingerabdruck und atomarer Austausch machen wiederholte Publikationen prüfbar und schützen eine bestehende Datei bei einem Fehler.
