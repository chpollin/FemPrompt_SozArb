...
FR-11 to FR-13 acceptance, as built: FR-11 renders the reading text with a built-in Markdown renderer, falling back to the abstract, then to an empty state. FR-12 highlights and steps through in-text matches in the open document and filters the corpus via the prebuilt `docs/data/fulltext_index.json`. FR-13 pins a selected passage or a search hit as `evidence[category] = {term, snippet, ts}` in the reviewer file (schema 0.2), sets the category, and reports an `evidence_count` in the decision log. The behaviour contract is in [[data]] (Evidence behaviour). As of P2 (ADR-024) the reading text is resolved through the single `fetchPaperText` seam in order, the raw local full text from the connected clone first (`generated/markdown_clean/`, never served), then the served knowledge document, then the abstract; the source actually read is recorded per decision as `text_source` and rendered as a pill. On public Pages without a connection the served knowledge document stays the reading text. Built, pending human browser verification (File System Access cannot be exercised headless).
...
### ADR-024 Raw local full text through the connected clone, with a recorded text source (implements ADR-013, extends the connect scope of ADR-021)

Kontext. Until now the screening view read only the served distilled knowledge document (ADR-013 planned the raw local text and left it open, plan.md P2/O4). Evidence pinned on a distillate inherits the distillate's framing, and the round-1 audit could not reconstruct which text a given decision was grounded on. The raw Docling full texts live in `generated/markdown_clean/`, outside the served `docs/` tree, copyrighted and never published. The connect target of the data panel (ADR-021) was `docs/data/screening/`, which cannot reach the raw texts one level up.

Wahl. The picker now connects the repo root of the local clone. The tool derives `docs/data/screening/` for the reviewer files (created if absent) and `generated/markdown_clean/` for the raw texts as nested subpaths, so the raw texts stay outside `docs/` and are never served. A folder without a `docs/` child is treated as the screening folder itself, a backward-compatible connection in which raw text is unavailable and the data panel says so. `fetchPaperText` stays the single seam and resolves a primary reading text in order, the raw local full text first, then the served knowledge document, then the abstract, then none; any miss or read error falls silently to the next link, so screening never breaks. The paper-to-rawfile mapping is title-prefix dominant, the raw filename's truncated title snippet must be a prefix of the paper's normalized title, with a year tiebreak and the longest snippet winning; the author is not used, because it diverges between the corpus metadata and the Docling filenames. Raw text renders as the Volltext (paper) layer, while the KI-Extraktion layer keeps coming from the knowledge document, so the ADR-016 layer-provenance split survives raw mode; a pill names the active source, and a raw-layer pin stays `origin: human`. Every decision records `text_source` (`raw`, `knowledge_doc`, `abstract`, `none`), rehydrated on edit, shown in the locked view, and emitted as a `text_source` column in the decision-log CSV. The change is additive and the reviewer schema stays `femprompt-prisma-reviewer/0.2`, since a pre-P2 file without `text_source` loads unchanged; this supersedes the schema-0.3 note in ADR-013 and plan.md P2.

Begründung. A Beleg should come from the paper rather than from an LLM summary of it, and recording the text source makes the evidence basis auditable, the actor-text-evidence triple the round-1 audit could not recover. Holding the raw texts behind the File System Access permission and out of `docs/` preserves the copyright boundary while public Pages degrades to the knowledge document. The title-prefix match is specific enough to need no shipped manifest, since the snippet is a long title prefix and two genuinely different papers rarely share it; keeping the schema additive lets existing reviewer files load without migration.

Effekt. Implemented in `docs/js/prisma.js` (resolveScopes and the repo-root connect, the `screeningHandle`/`rawHandle` split, parseRawName/matchRawFile/mapRawFilename, the fetchRawText/fetchKd/fetchPaperText resolution, the dual-layer loadReadingInto, the source pill, currentTextSource, the `text_source` in commit, editRecord, and the locked view, and the decision-log column) plus the data-panel copy. Extends the connect scope of ADR-021's data panel from the screening folder to the repo root and implements the raw-text half of ADR-013 (O4/P2). Test-covered (tests/tests.js Section P2, the mapping with its year and longest-prefix tiebreaks, the parse fallback, the currentTextSource fallback chain, the `text_source` recording and rehydration, and the CSV column); the headless harness is green. Built, pending human browser verification (File System Access connect and the raw read cannot be exercised headless). Over the current corpus the title-prefix match binds nearly every paper that carries a knowledge document to its raw file, the only misses being the raw files whose Docling name has no parseable year; a near-duplicate corpus entry binding to the same raw file as its sibling is correct, since they are the same underlying paper.
