...
| [[handoff]] | Current result of the working branch, continuation point for the next session, and conditions of working in a clone | per session close |
...
- Understand a tool decision: [[specification]] (the Decisions section with the ADR register, and the design system), [[journal]].
...
These documents describe the result and the decisions behind it, not how an agent session was organised. Lane, persona, and mission-control framing therefore stays out of the prose, while the content of an operator decision and its date remain recorded. Third parties appear by role and institution, and corpus works are cited by author and year or by title and Zotero record key.

Frontmatter such as `status: complete` describes the documentation artifact. It does not declare that the literature review, source coverage, distillation or manuscript is complete. Use the [completion package](../generated/completion/README.md), the [released Assertion index](../docs/data/assertion_index.json) and [[verification]] for those states. Dated ADRs and journal entries preserve their historical decisions and the figures recorded at their date. Current publication rules are in [[governance#Publication boundary]].
