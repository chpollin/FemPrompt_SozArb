---
source_url: https://www.ijcai.org/proceedings/2023/0742.pdf
authors:
  - Usman Gohar
  - Lu Cheng
doi: 10.24963/ijcai.2023/742
licence: "IJCAI permission to publish any version with a visible reference and link to the copyright holder; https://www.ijcai.org/past_conferences"
converter: docling with controlled visual repair
original_conversion_sha256: sha256:424ddf5785349b77ef328799e6969ffc425753ad9a6386aac154aaaca712e00d
---

Copyright and republication reference: © International Joint Conferences on Artificial Intelligence Organization. This repaired reading copy follows the [IJCAI permission to publish versions of the paper with a visible reference and link to IJCAI as copyright holder](https://www.ijcai.org/past_conferences).
...
<!-- Repair: Figure 1 transcribed from original PDF page 1. -->

> **Figure 1 text description.** A two-by-two matrix crosses race columns **Black** and **White** with gender rows **Male** and **Female**. The cells are A = Male and Black, B = Male and White, C = Female and Black, and D = Female and White. The figure defines independent groups as Male = A ∪ B, Female = C ∪ D, Black = A ∪ C, and White = B ∪ D. It defines intersectional groups as {A, B, C, D}. Gerrymandering groups comprise {Male, Female, Black, White, A, B, C, D}.
...
<!-- Repair: Figure 2 transcribed from original PDF page 2. -->

> **Figure 2 text description.** **Intersectionality** branches to **Fairness Notions** and **Mitigation**. Under **Fairness Notions**, **Subgroup Fairness** connects to **False Positive** and **Statistical Parity**; **Max-Min Fairness** stands as a separate notion; **Metric-based Fairness** connects to **Metric Multifairness** and **Probably Approximate**; **Probabilistic Fairness** stands as a separate notion; **Calibration** connects to **Multicalibration**, **Multiaccuracy**, and **Weighted Multicalibration**; and **Differential Fairness** stands as a separate notion. Under **Mitigation**, the branches are **Learning beyond surrogate fairness**, **Fairness with Demographics**, and **Fairness without Demographics**. **Fairness with Demographics** additionally connects to **Subgroup fairness via auditing**.
...
<!-- Repair: Equation (1) transcribed from original PDF page 2. -->

$$
\left|P(f(x)=1)-P(f(x)=1\mid c(s)=1)\right|\times P(c(s)=1)\leq\gamma. \tag{1}
$$
...
<!-- Repair: Equation (2) transcribed from original PDF page 2. -->

$$
\left|\mathbb{E}\left[c(x)\cdot(y-v)\mid f(x)=v\right]\right|\leq\alpha. \tag{2}
$$
...
<!-- Repair: unnumbered multiaccuracy constraint transcribed from original PDF page 2; the original inline conversion above is retained. -->

$$
\mathbb{E}\left[c(x)\cdot f(x)-y(x)\right]\leq\alpha\qquad\forall c\in C.
$$

 These notions define two extremes between efficiency and strong fairness guarantees. To find a balance between the two, [Gopalan et al. , 2022] introduces a hierarchy of weighted multicalibration. Formally:

Definition 3. Given C and a weight class W , f ( x ) is ( C , W , α ) -multicalibrated, if ∀ c ∈ C and ∀ w ∈ W

<!-- Repair: Equation (3) transcribed from original PDF page 2. -->

$$
\left|\mathbb{E}\left[c(x)\cdot w(f(x))(y-f(x))\right]\right|\leq\alpha. \tag{3}
$$
...
<!-- Repair: Figure 3 transcribed from original PDF page 3. -->

> **Figure 3 text description.** Four nested ellipses show the multicalibration hierarchy. From the largest outer region to the smallest inner region, they are **MC₁ = MA**, **MC₂**, **MCₖ**, and **MC**. An arrow carries the label **Increase in sample and computational complexity**, indicating that movement from multiaccuracy toward full multicalibration increases both requirements.
...
<!-- Repair: Equation (4) transcribed from original PDF page 3. -->

$$
\mathbb{P}_{(x,x')\sim D}\left[\left|f(x)-f(x')\right|\geq d(x,x')+\gamma\right]\leq\alpha. \tag{4}
$$
...
<!-- Repair: Equation (5) transcribed from original PDF page 3. -->

$$
\mathbb{E}_{(x,x')\sim A}\left[\left|f(x)-f(x')\right|\right]\leq\mathbb{E}_{(x,x')\sim A}\left[d(x,x')\right]+\gamma. \tag{5}
$$
...
Definition 7. For ϵ ≥ 0 and δ ∈ [0 , 1] , a predictor is ( ϵ, δ ) -probably intersectionally fair if

<!-- Repair: Equation (7) transcribed from original PDF page 4. -->

$$
P(U\geq\epsilon)\leq\delta. \tag{7}
$$


where U = u ( f ( x ) , s, s ′ ) measures unfairness for a randomly chosen prediction and two protected groups ( s ≠ s ′ ) to compare them. Probabilistic fairness captures the expected size δ of the population for which the predictor discriminates more than ϵ .
...
<!-- Repair: Equation (8) transcribed from original PDF page 5. -->

$$
L_{\max}(f)=\max_{s\in S}\mathbb{E}[l(f;X)]. \tag{8}
$$
...
<!-- Repair: Equation (9) transcribed from original PDF page 5. -->

$$
L_{\mathrm{DRO}}(f,r)=\sup_{Q\in B(P,r)}\mathbb{E}_{Q}[l(f;X)]. \tag{9}
$$
...
Current applications of intersectionality have been focused on NLP and Ranking. Some other promising applications include recommender systems [Islam et al. , 2019], graph em-


<!-- PAGE 7 -->


Table 1: Summary of popular datasets across different AI domains that contain multiple intersectional groups. Law School [Wightman, 1998] and Compass [Angwin et al. , 2016] are also used in Ranking. Adult [Kohavi, 1996], Student [Cortez and Silva, 2008], Psychometrics [Abbasi et al. , 2021], Multilingual Twitter Corpus (MTC) [Huang et al. , 2020], Five Item Personality Inventory (FIPI) and Myers-Briggs Type Indicator (MBTI) [Gjurkovi´ c et al. , 2021], MovieLens [Harper and Konstan, 2015], MEPS [Cohen et al. , 2009], CelebA [Liu et al. , 2015], UTKFace [Zhang et al. , 2017], PPB [Buolamwini and Gebru, 2018].

<!-- Repair: Table 1 row boundaries transcribed from original PDF page 7. -->

| Type | Dataset | Demographics |
|---|---|---|
| Tabular | Adult | Gender, Age, Race |
| Tabular | Student | Gender, Age, Alcohol, Relationship |
| Tabular | Law School | Gender, Age, Race, Income |
| Tabular | Compass | Gender, Race |
| NLP | Psychometrics | Gender, Age, Race, Income, Education |
| NLP | MTC | Gender, Age, Race |
| NLP | FIPI | Gender, Age, Race, Income, Education |
| NLP | MBTI | Gender, Age |
| Ranking | MEPS | Gender, Race, Age |
| Ranking | MovieLens | Gender, Age, Occupation |
| Image | CelebA | Gender, Age, Race |
| Image | UTKFace | Gender, Age, Ethnicity |
| Image | PPB | Gender, Race |

beddings [Bose and Hamilton, 2019], computer vision [Steed and Caliskan, 2021], and so on. It is imperative that existing MLsystems are holistically evaluated under the intersectional framework to aid in developing inclusive and fair ML systems. In NLP, a limitation of current works is the assumption that demographic information is available. Given increasing regulatory and privacy concerns, more research is needed to understand potential correlations in data that can be leveraged to tackle intersectional biases.
...
Next, we briefly discuss open problems and potential future research directions.
